 ...
 * individual contributors as indicated by the @author tags. See the copyright.txt
 * file in the distribution for a full listing of individual contributors. This is
 ...
 * software; if not, write to the Free Software Foundation, Inc., 51 Franklin St,
...
import org.wildfly.extras.jmxconsole.util.WildflyUtil;
...
public class Server {
	...
	public static Iterator<DomainData> getDomainData(String filter) throws JMException {
		...
		if (server != null) {
			...
			log.info("Querying ObjectNames: " + filter);
			...
			while (objectNamesIter.hasNext()) {
				...
				try {
					...
					// safely omit an MBean
					log.error("Exception inspecting MBean: " + name);
				}
			}
		}
		Iterator<DomainData> domainDataIter = domainData.values().iterator();
	...
	public static AttrResultInfo getMBeanAttributeResultInfo(String name,
			MBeanAttributeInfo attrInfo) throws JMException {
		...
		try {
			typeClass = WildflyUtil.getPrimitiveTypeForName(attrType);
	...
	public static AttributeList setAttributes(String name, HashMap attributes) throws JMException {
		...
		for (int a = 0; a < attributesInfo.length; a++) {
			...
			Attribute attr = null;
			try {
				Object realValue = WildflyUtil.convertValue(value, attrType);
	...
	public static OpResultInfo invokeOpByName(String name, String opName, String[] argTypes,
			String[] args) throws JMException {
		MBeanServer server = getMBeanServer();
		ObjectName objName = new ObjectName(name);
		...
		for (int p = 0; p < typedArgs.length; p++) {
			String arg = args[p];
			try {
				Object argValue = WildflyUtil.convertValue(arg, argTypes[p]);
